...
if(mysqli_num_rows($result) > 0) {
	while($row=mysqli_fetch_assoc($result)){
		...
		if ($result2 && mysqli_num_rows($result2) > 0) {
			...
			// Check if the 'time' key exists in $row2 before accessing it
			if (isset($row2['time'])) {
				$outtime = $row2['time'];
			} else {
				// Handle the case when 'time' key is not present in $row2
				$outtime = "N/A"; // Set a default value or handle the error in an appropriate way
			}
		} else {
			// Handle the case when no rows are returned or there's an error
			$outtime = "N/A"; // Set a default value or handle the error in an appropriate way
		}
