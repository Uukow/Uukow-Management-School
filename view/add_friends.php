...
if(mysqli_num_rows($result) > 0){
	while($row=mysqli_fetch_assoc($result)){
		...
		$status = null; // Default value if $row1['_status'] doesn't exist or $row1 is null

if ($row1 !== null && isset($row1['_status'])) {
    $status = $row1['_status'];
}

// Now you can use the $status variable for further processing
