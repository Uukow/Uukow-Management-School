	...
	while($row=mysqli_fetch_assoc($result)){
		...
		if($friend_type=="Student"){
			...
			if ($result1) {
				...
				if ($row1) {
					$friend_name = isset($row1['i_name']) ? $row1['i_name'] : null;
					$friend_image = isset($row1['image_name']) ? $row1['image_name'] : null;
			
					// Rest of your code using $friend_name and $friend_image
...
?>   
							...
							<td id="td1_<?php echo $row['mf_id']; ?>">
								<a href="#" onClick="friendProfile('<?php echo $friend_type; ?>','<?php echo $friend_index; ?>')">
